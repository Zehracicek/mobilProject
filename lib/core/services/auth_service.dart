...
class AuthService {
  ...
  Future<UserModel> register({
    ...
    try {
      ...
      final createdId = await _dbHelper.createUser(newUser);
      final createdUser = newUser.copyWith(id: createdId);

      await _saveUserSession(createdUser.copyWith(token: token), token);
  ...
  Future<UserModel> login({
    ...
    try {
      ...
      if (localUser != null && localUser.password == password) {
        ...
        await _saveUserSession(localUser.copyWith(token: fakeToken), fakeToken);
        return localUser;
      }

      throw Exception(
        'Kullanıcı bulunamadı veya şifre hatalı. Lütfen önce kayıt olun.',
      );
    } catch (e) {
  ...
  Future<void> logout() async {
    try {
      ...
      if (token != null) {
        try {
          await _httpService.post('/auth/logout', body: {}, token: token);
        } catch (e) {
          // Ignore network errors during logout, but log in debug mode
          if (kDebugMode) {
            // ignore: avoid_print
  ...
  Future<void> _saveUserSession(UserModel user, String token) async {
    ...
    await _localStorage.setObject(
      LocalStorageService.keyUserData,
      user.toMap(),
    );
